# YukiMod

Base template extracted from `MeiLinMod-sts2`.

This project keeps the playable-character scaffolding and placeholder visuals, but intentionally leaves cards, powers, relics, and potions empty.

## AI Collaboration Entry

- Start with `AGENTS.md`
- Project context: `docs/project-overview.zh-CN.md`
- Workflow rules: `docs/ai-workflow.zh-CN.md`
- Character brief: `docs/character-brief.zh-CN.md`
- Template cleanup list: `docs/template-cleanup-checklist.zh-CN.md`
