...
- 当前默认构建目标为 `109` 正式版，依赖 `STS2.RitsuLib` / 游戏 Mod `STS2-RitsuLib`，默认版本为 `0.4.59`
- `107`、`108` 仍保留为显式传参兼容构建路径；新工程适配和运行验证默认按 `109` 处理
- 当前已知原游戏解包参考目录仍为 `E:\DATA\GODOT\MyMod\sts104`，只能作为旧版行为参考；涉及当前 API 时以本机 109 游戏目录中的 `data_sts2_windows_x86_64/sts2.dll` 为准
...
- Godot 路径是否仍然有效
- 资源导出是否跟代码版本一致
- 如需核对原版能力、遗物、卡牌、动作或 Hook 行为，104 行为仍可先看 `E:\DATA\GODOT\MyMod\sts104`；109 API 签名需要以当前配置的 109 游戏目录 `data_sts2_windows_x86_64/sts2.dll` 为准
