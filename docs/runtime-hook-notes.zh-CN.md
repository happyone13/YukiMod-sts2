...
1. 可先看 `sts104` 原版调用链，再用目标 108 的 `sts2.dll` 校验 Hook 签名
...
`sts104/src/Core/Combat/CombatManager.cs` 中已确认；用于 108 正式版时还需要以目标版本 API 校验签名：
...
## 9. 异步动画回调必须同时校验整次请求代际

攻击动画通常同时等待 Spine 事件、动画结束信号和超时回退。只给其中某个阶段设置局部 watcher 编号还不够：后一轮攻击可能已经更新整次请求，但前一轮 Ready 阶段的旧事件仍满足局部状态条件。

Yuki 近战瞬移攻击的统一口径是：

1. 每次新的攻击请求递增 `LastRequestId`。
2. Ready / Attack / Return 各阶段可以保留自己的 watcher 编号，用于淘汰同一请求内的旧阶段回调。
3. 所有延迟回调必须同时校验 `LastRequestId == requestId` 和当前阶段的 watcher / state；两者任一不匹配就直接返回。
4. Spine 事件、动画完成信号、定时回退必须使用同一组校验条件，不能只保护其中一条路径。

这样后一轮攻击一旦开始，前一轮尚未到达的命中、回位或待机回调就会自然失效，不会干扰新时间轴。

瞬移近战还必须把绘制层级纳入同一会话快照：第一次前移前记录角色节点的父节点、sibling index、`ZIndex` 与 `ZAsRelative`；到达敌方位置时临时移动到其他战斗单位之上；正常回程、动画完成回程、原版动画回退和大招打断旧近战会话时，都先恢复这份快照。不能只恢复 `GlobalPosition`，否则角色虽然回到原位，绘制顺序仍可能永久留在敌人后方或前方。

启动阶段的 VFX 预热应先对路径去重，并记录 `requested / loaded / failed` 汇总。单条缺失路径日志用于定位资源，汇总日志用于判断本次发布是否存在系统性漏包；不要用“初始化没有抛异常”替代资源覆盖率检查。

资源加载预热之后，Yuki 的高频瞬移/攻击 VFX 还会在 `NCombatRoom._Ready` 后做分帧深度预热：逐个实例化场景，在真实 `CombatVfxContainer` 中以 `0.001` 透明度播放可用的 `animation / eff_b / eff_f`，提交两帧后释放，再让出一帧处理下一个场景。任务必须同时校验房间实例、generation、特效设置和容器有效性；新战斗开始或设置关闭后，上一任务不可继续操作场景树。
