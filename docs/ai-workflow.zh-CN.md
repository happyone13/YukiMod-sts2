...
- 涉及原版 `104` 行为时，优先查看解包目录 `E:\DATA\GODOT\MyMod\sts104` 作为旧版参考；涉及当前 109 构建的 API 签名时，以配置的 109 游戏目录中的 `data_sts2_windows_x86_64/sts2.dll` 为准
- 涉及“回合开始”“抽牌前/后”“重抽整手牌”“是否计入起手抽牌”等效果时，先对照对应目标版本的 `CombatManager.cs` 实际调用顺序；只有 104 资料时可先参考 `sts104/src/Core/Combat/CombatManager.cs`，再用 109 API 校验
