...
- 涉及原版 `104` 行为时，优先查看解包目录 `E:\DATA\GODOT\MyMod\sts104` 作为旧版参考；涉及当前 107 构建的 API 签名时，以配置的 107 游戏目录中的 `data_sts2_windows_x86_64/sts2.dll` 为准
...
- 当前 RitsuLib 注册会把友纪公开 ID 规范化为 `YUKIMOD_...`；代码常量、补丁判断和本地化 key 必须使用下划线格式，例如 `YUKIMOD_STRIKE_YUKI.title`，不要再写旧的 `YUKIMOD-...`
- 新文本文件默认使用 `UTF-8`
